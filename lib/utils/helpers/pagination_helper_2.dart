...
class PaginationHelper2<T> {
  ...
  PaginationHelper2({required this.asyncTask, int? limit, this.listener, this.onRefresh}) {
    this.limit = limit ?? 14;
  }
  ...
  Future<void> run() {
    ...
    return asyncTask.call(_config).then((value) {
      ...
      bsIsLoadingMore.add(false);
      if (e.toString().contains('ProviderSubscription.read()')) {
      } else {
        throw e;
      }
    });
